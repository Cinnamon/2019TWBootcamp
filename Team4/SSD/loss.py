...
class MultiBoxLoss_2(nn.Module):
    num_classes = 2

    def __init__(self):
        super(MultiBoxLoss_2, self).__init__()
    ...
    def forward(self, loc_preds, loc_targets, conf_preds, conf_targets):
        ...
        ################################################################
        conf_loss = F.cross_entropy(conf_preds.view(-1,self.num_classes),                                             conf_targets.view(-1), reduce=False)  # [N*8732,]
        neg = self.hard_negative_mining(conf_loss, pos)    # [N,8732]

        pos_mask = pos.unsqueeze(2).expand_as(conf_preds)  # [N,8732,2]
        neg_mask = neg.unsqueeze(2).expand_as(conf_preds)  # [N,8732,2]
        mask = (pos_mask+neg_mask).gt(0)

        pos_and_neg = (pos+neg).gt(0)
        preds = conf_preds[mask].view(-1,self.num_classes)  # [#pos+#neg,2]
        ...
        #print('loc_loss: ', loc_loss.data)
        conf_loss /= num_matched_boxes
        #print('conf_loss: ', conf_loss.data)
...
get_ipython().system('jupyter nbconvert --to script loss.ipynb')
